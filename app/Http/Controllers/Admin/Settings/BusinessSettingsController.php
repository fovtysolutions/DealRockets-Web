...
class BusinessSettingsController extends BaseController
{
    ...
    public function getView(): View
    {
        ...
        $settingsu = $this->getSettings($web, 'colorsu');
        $datau = $settingsu ? json_decode($settingsu['value'], true) : [
            'textcolor' => '#000000',
            'hovercolor' => '#007BFF',
        ];

        $settingsp = $this->getSettings($web, 'colorsp');
        $datap = $settingsp ? json_decode($settingsp['value'], true) : [
            'bgcolor' => '#000000',
        ];

        $settingsl = $this->getSettings($web, 'headercolors');
        $datal = $settingsl ? json_decode($settingsl['value'], true) : [
            'first_header_color' => '#FFFFFF',
            'second_header_color' => '#FFFFFF',
        ];
        ...
        $businessSetting = [
            ...
            'text_color' => $datau['textcolor'] ?? '',
            'hover_color' => $datau['hovercolor'] ?? '',
            'bg_color' => $datap['bgcolor'] ?? '',
            'first_header_color' => $datal['first_header_color'] ?? '#FFFFFF',
            'second_header_color' => $datal['second_header_color'] ?? '#FFFFFF',
            ...
            'banner_logo' => $this->getSettings(object: $web, type: 'company_banner_logo')->value ?? '',
    ...
    public function updateSettings(BusinessSettingRequest $request, BusinessSettingService $businessSettingService): RedirectResponse
    {
        ...
        $colorsl = json_encode(['first_header_color' => $request['first_header_color'], 'second_header_color' => $request['second_header_color']]);
        $this->businessSettingRepo->updateOrInsert(type: 'headercolors', value: $colorsl);

        $colorsu = json_encode(['textcolor' => $request['textcolor'], 'hovercolor' => $request['hovercolor'], 'primary_light' => $request['primary_light'] ?? '#CFDFFB']);
        $this->businessSettingRepo->updateOrInsert(type: 'colorsu', value: $colorsu);

        $colorsp = json_encode(['bgcolor' => $request['bgcolor'], 'primary_light' => $request['primary_light'] ?? '#CFDFFB']);
        $this->businessSettingRepo->updateOrInsert(type: 'colorsp', value: $colorsp);
        ...
        $bannerImage = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'company_banner_logo']);
        if ($request->has('company_banner_logo')) {
            $bannerIconImage = [
                'image_name' => $this->updateFile(dir: 'company/', oldImage: (is_array($bannerImage['value']) ? $bannerImage['value']['image_name'] : $bannerImage['value']), format: 'webp', image: $request->file('company_banner_logo')),
                'storage' => config('filesystems.disks.default') ?? 'public'
            ];
            $this->businessSettingRepo->updateWhere(params: ['type' => 'company_banner_logo'], data: ['value' => $bannerIconImage]);
        }
        $bannerImage1 = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'company_banner_logo1']);
        if ($request->has('company_banner_logo1')) {
            $bannerIconImage1 = [
                'image_name' => $this->updateFile(dir: 'company/', oldImage: (is_array($bannerImage1['value']) ? $bannerImage1['value']['image_name'] : $bannerImage1['value']), format: 'webp', image: $request->file('company_banner_logo1')),
                'storage' => config('filesystems.disks.default') ?? 'public'
            ];
            $this->businessSettingRepo->updateWhere(params: ['type' => 'company_banner_logo1'], data: ['value' => $bannerIconImage1]);
        }
        $bannerImage2 = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'company_banner_logo2']);
        if ($request->has('company_banner_logo2')) {
            $bannerIconImage2 = [
                'image_name' => $this->updateFile(dir: 'company/', oldImage: (is_array($bannerImage2['value']) ? $bannerImage2['value']['image_name'] : $bannerImage2['value']), format: 'webp', image: $request->file('company_banner_logo2')),
                'storage' => config('filesystems.disks.default') ?? 'public'
            ];
            $this->businessSettingRepo->updateWhere(params: ['type' => 'company_banner_logo2'], data: ['value' => $bannerIconImage2]);
        }
        $bannerImage3 = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'company_banner_logo3']);
        if ($request->has('company_banner_logo3')) {
            $bannerIconImage3 = [
                'image_name' => $this->updateFile(dir: 'company/', oldImage: (is_array($bannerImage3['value']) ? $bannerImage3['value']['image_name'] : $bannerImage3['value']), format: 'webp', image: $request->file('company_banner_logo3')),
                'storage' => config('filesystems.disks.default') ?? 'public'
            ];

...
class CustomerAuthController extends Controller
{
    ...
    {
        $this->customerAuthService->checkRecaptchaValidation(request: $request);
    ...
    public function resendOTPCode(Request $request): JsonResponse|RedirectResponse
    {
        ...
        if ($timeDifferance == 0) {
            ...
            if ($email_verification && !$user['is_email_verified']) {
                $email_services_smtp = getWebConfig(name: 'mail_config');
                if ($email_services_smtp['status'] == 0) {
                    $email_services_smtp = getWebConfig(name: 'mail_config_sendgrid');
                }

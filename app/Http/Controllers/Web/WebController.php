...
class WebController extends Controller
{
    ...
    public function symboliclink(){
        $target = storage_path('app/public'); // The storage directory
        $link = public_path('storage'); // The public directory
        ...
        if (symlink($target, $link)) {
            ...
            return "Failed to create the symbolic link.";
        }    
    }
    ...
    {
        // Fetch all states for the given country
        ...
        // Initialize an array to collect city data
        $cities = [];
    
        // Loop through each state and fetch its cities
        foreach ($states as $state) {
            $stateCities = City::where('state_id', $state->id)->get(['id', 'name']);
            $cities = array_merge($cities, $stateCities->toArray()); // Merge city data into the array
        }

...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            'additional_details' => isset($request['dynamic_data']) ? json_encode($request['dynamic_data']) : json_encode([]),
            ...
            'technical_specification' => isset($request['dynamic_data_technical']) ? json_encode($request['dynamic_data_technical']) : json_encode([]),
            'supply_capacity' => $request['supply_capacity'] ?? null,
            'supply_unit' => $request['supply_unit'] ?? null,
            'local_currency' => $request['local_currency'] ?? null,
        ...
        foreach ($collections as $collection) {
            foreach ($collection as $key => $value) {
                if ($key != "" && !in_array($key, $columnKey)) {
                    return [
                        'status' => false,
                        'message' => translate('Incorrect format: ' . $key . ' is not a valid field.'),
                        'products' => []
                    ];
                }
                ...
                if ($key != "" && $value === "" && !in_array($key, $skip)) {
                    return [
                        'status' => false,
                        'message' => translate('Please fill the ' . $key . ' field.'),
                        'products' => []
                    ];
                }
            }

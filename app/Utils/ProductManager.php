    ...
    public static function getProductListData($request, $productUserID = null, $productAddedBy = null): mixed
    {
        ...
        $productListData = Product::active()
            ...
            ->when(!empty($searchInput),function($query) use ($searchInput){
                return $query->where(function($q) use ($searchInput){
            ...
            ->when($request['data_from'] == 'category' && $request['category_id'], function ($query) use ($request) {
                return $query->where(['category_id' => $request['category_id']])
                    ->orWhere(function ($query) use ($request) {
                        $query->where('sub_category_id', $request['category_id']);
                    })
                    ->orWhere(function ($query) use ($request) {
                        $query->where('sub_sub_category_id', $request['category_id']);
                    });
            })
            ->when($request->has('category_ids') && !empty($request['category_ids']) && is_array($request['category_ids']), function ($query) use ($request) {
                return $query->whereIn('category_id', $request['category_ids'])
                    ->orWhere(function ($query) use ($request) {
                        $query->whereIn('sub_category_id', $request['category_ids']);
                    })
                    ->orWhere(function ($query) use ($request) {
                        $query->whereIn('sub_sub_category_id', $request['category_ids']);
                    });
            })

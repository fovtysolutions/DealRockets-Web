...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const dropdownButton = document.getElementById('cartDropdownButton');
        const dropdownMenu = document.getElementById('cartDropdownMenu');
        const dropdownContainer = document.getElementById('cartDropdownContainer');
...
<div class="cartheader d-flex navbar-tool dropdown me-2 {{ Session::get('direction') === 'rtl' ? 'mr-md-1' : 'ml-md-1' }}"
    id="cartDropdownContainer">
    @if ($web_config['guest_checkout_status'] || auth('customer')->check())
    <a href="{{ route('shop-cart') }}" class="d-inline-flex align-items-center">
                    <a class="navbaricons" href="#">
                        <div class="navbariconscontainer">
                            <span class="customcss navbar-tool-label">
                                @php($cart = \App\Utils\CartManager::get_cart())
                                {{ $cart->count() }}
                            </span>
                            <i class="headericon bx bx-cart" aria-hidden="true"></i>
                            <div class="inquiry-cart-label custom-dealrock-text">Cart</div>
                        </div>
                    </a>
                    {{-- <button style="color: {{ $textcolor }} !important;" class="border-0 bg-transparent fs-5" id="cartDropdownButton" style="cursor: pointer;" title="Cart">
                            <i class="fa-solid fa-cart-shopping"></i>
                        </button> --}}
                </a>
            @else
    <a href="{{ route('customer.auth.login') }}" class="d-inline-flex align-items-center">
        <a class="navbaricons" href="#">
            <div class="navbariconscontainer">
                <span class="customcss navbar-tool-label">
                    @php($cart = \App\Utils\CartManager::get_cart())
                            ...
                            @if(isset($product->status) && $product->status == 1)
                                <div class="__quantity">
                                    ...
                                    <input type="text"
                                        ...
                                        id="cartQuantity{{$cartItem['id']}}" data-cart-id="{{ $cartItem['id'] }}"
                                        data-product-id="{{ $cartItem['product_id'] }}" data-action="0" data-event=""

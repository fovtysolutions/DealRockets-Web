...
$colorat = ChatManager::getimagecolorat('storage/' . ($homepagesetting[0]['background_image'] ?? ''), 15, 15) ?? $defaultColor;
...
$getcategoryid = optional(Category::where('name', $homepagesetting[0]['category_title'] ?? '')->first())->id ?? 0; // default ID 1
?>
...
<section>
    <div class="sourcenow-sec">
        <div class="sourcemaindiv d-flex">
            <!-- Left Banner Section -->
            <div class="sourcefirstdiv bg-shimmer" data-bg='{{ 'storage/' . ($homepagesetting[0]['background_image'] ?? 'default-bg.jpg') }}'>
                <div class="cnt-1">
                    <div class="text-overlay">
                        <h4 class="title fourthshow" style="color:{{$text_color}} !important; font-size: 20px;">
                            {{ $homepagesetting[0]['category_title'] ?? 'Default Category' }}
                        </h4>
                        ...
                        <a class="btn source-btn btn-sm" style="background-color: white !important; color:#FE4E44 !important;"
                            ...
                            {{$homepagesetting[0]['button_text'] ?? 'Browse Products'}}
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="sourceseconddiv">
                @if (!empty($homepagesetting[0]['products']))
                    <div class="prd-grid firstshow">
                        ...
                        for ($i = 0; $i < 8; $i++) {
                            $product = $homepagesetting[0]['products'][$i] ?? null;
                            if ($product) {
                        ?>
                                <div class="prd-item">

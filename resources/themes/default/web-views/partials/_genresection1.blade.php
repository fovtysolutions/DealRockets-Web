...
$colorat = ChatManager::getimagecolorat('storage/' . ($homepagesetting[1]['background_image'] ?? ''), 15, 15) ?? $defaultColor;
...
$getcategoryid = optional(Category::where('name', $homepagesetting[1]['category_title'] ?? '')->first())->id ?? 0; // default ID 1
?>
...
<section>
    <div class="sourcenow-sec">
        <div class="sourcemaindiv d-flex">
            <!-- Left Banner Section -->
            <div class="sourcefirstdiv bg-shimmer" data-bg='{{ 'storage/' . ($homepagesetting[1]['background_image'] ?? 'default-bg.jpg') }}'>
                <div class="cnt-1">
                    <div class="text-overlay">
                        <h4 class="title fourthshow" style="color:{{$text_color}} !important; font-size: 20px;">
                            {{ $homepagesetting[1]['category_title'] ?? 'Default Category' }}
                        </h4>
                        ...
                        <a class="btn source-btn btn-sm" style="background-color: white !important; color:#FE4E44 !important;"
                            ...
                            {{$homepagesetting[1]['button_text'] ?? 'Browse Products'}}
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <div class="prd-item">
                                    <a style="all: unset; cursor: pointer;" href="{{ route('products', ['searchInput' => $product['name']]) }}" class="text-decoration-none">
                                        <div class="source-product-item image-wrapper shimmer">
                                            ...
                                            <img data-src="storage/{{ $product['image'] }}" alt="product" class="source-product-img lazyload align-self-end" />
                                        </div>
                                    </a>
                                </div>
                        <?php

@foreach ($chatdata as $item)
    <div @if ($item['is_read'] == 1) style="background-color: #d3d3d394;" @endif data-user-id="{{ $item['receiver_id'] }}" data-user-type="{{ $item['receiver_type'] }}" data-type="{{ $item['type'] }}" data-listing-id="{{ $item['leads_id'] }}" data-sendToId="{{ $item['sender_id'] }}" data-sendToType="{{ $item['sender_type'] }}"
        ...
        <div class="flex leading-none items-center py-1 px-2 w-[70%] h-full justify-between">
            <div class="flex w-[98%] overflow-hidden text-ellipsis whitespace-nowrap">
                <p class="text-black font-semibold">
                    ...
                    got a Inquiry from RFQ: {{ $item['message'] }}</p>
            </div>
            <p class="w-[2%]">...</p>
        </div>
        <!-- mail right section  -->

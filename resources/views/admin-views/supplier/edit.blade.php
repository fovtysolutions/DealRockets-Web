...
@section('content')
    <div class="content container-fluid">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{ translate('Update_Supplier') }}
            </h2>
        </div>

        <form class="product-form text-start" action="{{ route('admin.suppliers.update', $supplier->id) }}" 
                ...
                <div class="px-4 pt-3 d-flex justify-content-between">
                    <ul class="nav nav-tabs w-fit-content mb-4">
                        @foreach ($languages as $lang)
                            <li class="nav-item">
                                <span class="nav-link text-capitalize form-system-language-tab {{ $lang == $defaultLanguage ? 'active' : '' }} cursor-pointer" id="{{ $lang }}-link">
                        ...
                        <div class="{{ $lang != $defaultLanguage ? 'd-none' : '' }} form-system-language-form" id="{{ $lang }}-form">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" {{ $lang == $defaultLanguage ? 'required' : '' }} name="name" id="{{ $lang }}_name"
                                    ...
                                    placeholder="{{ translate('new_Supplier') }}" value="{{ old('name', $supplier->name) }}">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="js-select2-custom form-control action-get-request-onchange" name="business_type"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category['id'] }}" {{ $supplier->business_type == $category['id'] ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="main_products" id="{{ $lang }}_main_products" class="form-control"
                                    placeholder="{{ translate('enter_main_products') }}" value="{{ old('main_products', $supplier->main_products) }}">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="management_certification" id="{{ $lang }}_management_certification" class="form-control"
                                    placeholder="{{ translate('enter_management_certification') }}" value="{{ old('management_certification', $supplier->management_certification) }}">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" {{ $lang == $defaultLanguage ? 'required' : '' }} name="city_province"
                                    id="{{ $lang }}_city_province" class="form-control" placeholder="{{ translate('enter_city_province') }}" value="{{ old('city_province', $supplier->city_province) }}">
                            </div>

                            <!-- Image Uploads -->
                            @for ($i = 1; $i <= 3; $i++)
                                <div class="form-group">
                                    <label class="title-color" for="{{ $lang }}_image{{ $i }}">{{ translate('image_' . $i) }}</label>
                                    <input type="file" name="image{{ $i }}" id="{{ $lang }}_image{{ $i }}" class="form-control" accept="image/*" onchange="previewImage(event, '{{ $lang }}_image{{ $i }}_preview')">
                                    @if ($supplier->{'image' . $i})
                                        <img style="width: 300px; height: 300px;" id="{{ $lang }}_image{{ $i }}_preview" src="{{ asset('/storage/' . $supplier->{'image' . $i}) }}" class="image-preview" onclick="document.getElementById('{{ $lang }}_image{{ $i }}').click();">
                                    @else
                                        <img id="{{ $lang }}_image{{ $i }}_preview" class="image-preview" style="display: none;">
                                    @endif
                                </div>
                            @endfor
                        </div>
    ...
    <span id="image-path-of-product-upload-icon" data-path="{{ dynamicAsset('public/assets/back-end/img/icons/product-upload-icon.svg') }}"></span>
    <span id="image-path-of-product-upload-icon-two" data-path="{{ dynamicAsset('public/assets/back-end/img/400x400/img2.jpg') }}"></span>

...
@section('title')
Buyers - {{ $leadrequest->name }} | {{ $web_config['name']->value }}
@endsection
@section('content')
<?php
...
<div class="mainpagesection">
    ...
    <div class="card border-0 rounded-0" style="background-color: var(--web-bg);">
        <span> <a href="/"> Home </a> / <a href="/buyers"> Buyer </a> / {{$leadrequest->name}}</span>
    </div>
    {{-- <div class="gapbetweens border-0 rounded-0">
        <!-- Empty Gap -->
    </div>
        ...
        <div class="egrrgr">
            ...
            <div class="hrrgr">
                <div class="main-search" style="width: 100%;">
                    <form action="{{ route('buyer') }}" method="GET" id="header_search_bar">
            ...
            <div class="carddetails">
                            ...
                            <ul class="nleftattr">
                                ...
                                @if (auth('customer')->check() && auth('customer')->user()->id)
                                    @if ($membership['status'] == 'active')
                                        <li>
                                            <span>Contact Number</span><a class="p-0 m-0" style="line-height: 1;">{{ $leadrequest->contact_number }}</a>
                                        </li>
                                        <li><span>Company Name</span><a class="p-0 m-0" style="line-height: 1;">{{ $leadrequest->company_name }}</a>
                                        </li>
                                    @else
                                        <li>
                                            <span>Contact Number</span><a href="{{ route('membership') }}" class="txt_link p-0 m-0" style="line-height: 1;"
                                            title="Become a member to view Company Name">Show</a>
                                        </li>
                                        <li><span>Company Name</span><a href="{{ route('membership') }}" class="txt_link p-0 m-0" style="line-height: 1;"
                                            title="Become a member to view Mobile Number">Show</a>
                                        </li>
                                    @endif
                                @else
                                <li><span>Contact Number</span><a href="javascript:void(0)" class="txt_link p-0 m-0" style="line-height: 1;"
                                        title="Login in to view Company Name" onclick="openLoginModal()">Show</a>
                                </li>
                                <li><span>Company Name</span><a href="javascript:void(0)" class="txt_link p-0 m-0" style="line-height: 1;"
                                        title="Login in to view Mobile Number" onclick="openLoginModal()">Show</a>
                                </li>
                                @endif
                                @php
                                ...
                                <li>Quantity Required <span>{{ $leadrequest->quantity_required }} {{$leadrequest->unit}}</span> </li>
                                <li>Buying Frequency <span class="text-capitalize">{{ $leadrequest->buying_frequency }}
                                    </span> </li>
                                </div>
                                <li>Term<span class="text-capitalize">{{$leadrequest->term}}</span></li>
                            </ul>

                            <h6 class="rel-wrapper">
                                Buy Offer Details
                            </h6>
                            <p class="b-desc">{{ $leadrequest->details }}</p>
                                @if (auth('customer')->check() && auth('customer')->user()->id)
                                    @if ($membership['status'] == 'active')
                                        <a style="margin-top: 20px !important;" href="#" class="view-buyers-btn text-white"
                                            ...
                                            data-leads-id="{{ $leadrequest->id }}" data-typereq="leads"
                ...
                <div class="detailright">
                    <a class="view-buyers-btn text-white" title="View All buyers from Netherlands"
                        ...
                        View All Buyers from {{ $countryDetails['countryName'] }}
                    </a>
                </div>
            </div>
        </div>
...
<script>
    function SearchbyCountry(id) {
        if (id) {
            window.location.href = '/buyers?country=' + id;
        } else {

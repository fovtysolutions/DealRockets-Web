...
@section('title')
Sellers - {{ $leadrequest->name }}  | {{ $web_config['name']->value }}
@endsection
@section('content')
<?php
...
<div class="mainpagesection">
    ...
    <div class="card border-0 rounded-0" style="background-color: var(--web-bg);">
        <span> <a href="/"> Home </a> / <a href="/sellers"> Seller </a> / {{$leadrequest->name}}</span>
    </div>
    {{-- <div class="gapbetweens border-0 rounded-0">
        <!-- Empty Gap -->
    </div>
            ...
            <div class="carddetails">
                ...
                <div class="detailright">
                    <a class="view-buyers-btn text-white" title="View All buyers from Netherlands"
                        ...
                        View All Sellers from {{ $countryDetails['countryName'] }}
                    </a>
                </div>
            </div>
        </div>
...
<script>
    function SearchbyCountry(id) {
        if (id) {
            window.location.href = '/sellers?country=' + id;
        } else {

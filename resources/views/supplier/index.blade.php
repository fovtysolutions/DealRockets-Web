...
<link rel="stylesheet" href="{{ theme_asset('public/assets/custom-css/supplier.css')}}" />
@section('title', translate($data['data_from']) . ' ' . translate('supplier'). ' | ' . $web_config['name']->value)
    ...
    <div class="search-page-header">
        <div>
            <h5 class="font-semibold mb-1 text-capitalize">
                ...
                {{ translate('suppliers') }} {{ isset($data['brand_name']) ? '(' . $data['brand_name'] . ')' : ''}}
            </h5>
            <div><span class="view-page-item-count">{{$totalsuppliers}}</span> {{translate('items_found')}}</div>
        </div>
        <form id="search-form" class="d-none d-lg-block" action="{{ route('supplier') }}" method="GET">
...
<div class="container pb-5 mb-2 mb-md-4 rtl __inline-35" dir="{{Session::get('direction')}}">
    <a class="listtab">
        <ul class="tab">
            ...
            <li id="supplierlist" class="selected">
                Supplier List
            </li>
        </ul>
    </a>
    <div class="row">
        ...
        <section class="col-lg-9">
            <div class="row" id="ajax-products">
                @include('supplier._ajax-products', ['products' => $products, 'decimal_point_settings' => $decimal_point_settings])
            </div>
        </section>
    </div>
</div>
